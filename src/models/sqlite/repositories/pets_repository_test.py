        ...
        self.session = UnifiedAlchemyMagicMock(
            data=[
                (
                    ...
                    [
                        PetsTable(name="Clodoaldo", type="dog"),
                     PetsTable(name="Greta", type="cat")
                     ] # resultado
...
def test_list_pets():
    ...
    mock_connection.session.query.assert_called_once_with(PetsTable) # Testando se é a PEts Table
    ...
    assert response[0].name == "Clodoaldo"
...
def test_list_pets_no_result():
    ...
    mock_connection.session.query.assert_called_once_with(PetsTable) # Testando se é a PEts Table
    mock_connection.session.all.assert_not_called() # Testando se não ta usando all()
    mock_connection.session.filter.assert_not_called() # Não há uso de filtro
